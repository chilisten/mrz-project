...
function Header({ screen, onGoHome, onGoProfile }) {
  ...
  return (
    <header className="relative z-20 px-4 py-3 sticky top-0"
      style={{ background: headerBg, borderBottom: `1px solid ${headerBd}`,
               boxShadow: isLight ? '0 1px 8px rgba(0,0,0,0.06)' : 'none', backdropFilter: 'blur(16px)' }}>
                ...
                <div className="absolute right-0 top-full mt-1 rounded-xl p-1 z-50 min-w-[130px]"
                  ...
                  {LANGUAGES.map(l => (
                    <button key={l.code} onClick={() => { setLang(l.code); setShowLangMenu(false) }}
                      ...
                      style={lang === l.code ? { background: 'rgba(14,165,233,0.15)', color: '#0ea5e9' } : { color: muted }}>
                      {l.flag} {l.label}
                    </button>
                  ))}
                </div>
...
function AppInner() {
  ...
  // какую вкладку открыть в профиле
  const [profileTab, setProfileTab] = useState('passports')

  const handleAuthSuccess = () => {
    if (authIntent === 'booking' && selectedFlight) {
      setScreen('booking')
  ...
  // Переход в историю броней — открывает профиль сразу на вкладке history
    ...
    <div className="min-h-screen font-body relative overflow-x-hidden"
      ...
      {!isLight && (
        <div className="fixed inset-0 pointer-events-none">
          <div className="absolute top-0 left-1/4 w-96 h-96 rounded-full blur-3xl" style={{ background: 'rgba(14,165,233,0.05)' }} />
          <div className="absolute bottom-0 right-1/4 w-64 h-64 rounded-full blur-3xl" style={{ background: 'rgba(99,179,237,0.05)' }} />
          <div className="absolute top-1/2 left-0 w-48 h-48 rounded-full blur-3xl" style={{ background: 'rgba(99,102,241,0.04)' }} />
        </div>
      )}

      {/* Auth screen — поверх всего, с кнопкой назад */}
      {screen === 'auth' && (
        <AuthScreen
      ...
      {screen !== 'auth' && (
        <>
          ...
          <main className="relative z-10 max-w-5xl mx-auto px-4 py-6">
            ...
            {screen === 'profile' && user && (
              <ProfileScreen initialTab={profileTab} onLogoutSuccess={() => setScreen('home')} />
            )}
            {screen === 'booking' && selectedFlight && user && (
              <BookingFlow
            ...
            {screen === 'confirmation' && lastBooking && (
              <ConfirmationScreen
                booking={lastBooking}
                tickets={lastTickets}
                flight={selectedFlight}
                onNewBooking={() => { setSelectedFlight(null); setLastBooking(null); setLastTickets(null); setScreen('home') }}
