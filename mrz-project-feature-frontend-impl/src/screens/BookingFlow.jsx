...
export default function BookingFlow({ flight, onBack, onComplete }) {
  const { tr, passports, addPassport, addBooking, theme, user } = useApp() // <-- ДОБАВЛЕН USER
  ...
  const handleConfirm = async () => {
    try {
      const formattedPassengers = passengers.map((p, i) => {
        const seatId = seatAssignments[i]
        const seatObj = seatObjects[seatId]
        return {
          passport_number: p.passport?.docNum || "AN000000",
          first_name: p.passport?.names || "SACHA",
          last_name: p.passport?.surname || "STUDENT",
          country_code: p.passport?.country || "KGZ",
          birth_date: p.passport?.birthDate || "2000-01-01",
          expiry_date: p.passport?.expiryDate || "2030-01-01",
          gender: p.passport?.gender || "M",
          seat_number: seatId,
          seat_class: seatObj?.class || 'economy'
        }
      })

      await amadeusService.createBooking(
        user?.id || 1,
        flight.id,
        formattedPassengers
      )
